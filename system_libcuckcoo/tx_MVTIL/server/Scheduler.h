...
class Scheduler : virtual public DataServiceIf {
  ...
  private:
	...
	typedef std::unordered_map<Key, Timestamp> ReadItem;
	typedef std::unordered_map<Key, std::pair<Value, Timestamp>> WriteItem;
	typedef cuckoohash_map<TransactionId, std::shared_ptr<WriteItem>, CityHasher<TransactionId>> WriteMap;
	typedef cuckoohash_map<TransactionId, std::shared_ptr<ReadItem>, CityHasher<TransactionId>> ReadMap;

...
server_machine = 3
client_machine = 5
s_hosts = running_instances_ip[:server_machine]
c_hosts = running_instances_ip[:client_machine]
...
def client_start(alg,test_id,c_num,tx_info):
	connect(c_hosts)
...
def execute_server_command(alg,param):
	command = 'fab -f run_ec2.py server_start:alg=%s,param=%s'%(alg,param)
	subprocess.Popen(command, shell=True)

def execute_client_command(alg,test_id,c_num):
	#command = 'fab -f run_lpd2.py client_start:alg=%s,test_id=%s,c_num=%s'%(alg,test_id,c_num)
	#result = subprocess.check_output(command, shell=True)
	tx_info = []
	client_start(alg,test_id,c_num,tx_info)
	...
	try:
		for test_id,test_name in test_cases.items():
			...
			for num in range(len(algs)):
				...
				for c_num in client_threads:
					commit_tx,total_tx = execute_client_command(test_alg,test_id,c_num)
					f.write('%d %d %d\n'%(c_num,commit_tx,total_tx))
					#f.write(str(c_num)+" "+str(commit_tx)+" "+str(total_tx)+'\n')
					f.flush()
				f.write("\n\n")
		f.close()
